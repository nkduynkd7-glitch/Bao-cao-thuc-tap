# PHẦN A – CHUẨN BỊ
## 1. Cài jq (bắt buộc để parse API)
```yaml
apt update
apt install -y jq
```
jq --version

## 2. Test API lấy VM toàn cluster (rất quan trọng)
```yaml
pvesh get /cluster/resources --type vm --output-format json | jq -r '.[].vmid'
```
Nếu bước này OK → đi tiếp.

# PHẦN B – SCRIPT ADD VM VÀO HA
## 3. Tạo thư mục script
```yaml
mkdir -p /root/scripts
```
## 4. Tạo file script
```yaml
nano /root/scripts/add_vm_to_ha.sh
```
## 5. Cấp quyền chạy
```yaml
chmod +x /root/scripts/add_vm_to_ha.sh
```
## 6. Test chạy tay (rất nên làm 1 lần)
```yaml
/root/scripts/add_vm_to_ha.sh
```
Sau đó kiểm tra:
```yaml
ha-manager status
```
# PHẦN C – SYSTEMD TIMER (02:00 SÁNG)
## 7. Tạo service
```yaml
nano /etc/systemd/system/add-vm-to-ha.service
```

Dán:
```yaml
[Unit]
Description=Add VM to Proxmox HA

[Service]
Type=oneshot
ExecStart=/root/scripts/add_vm_to_ha.sh
```
## 8. Tạo timer (02:00 mỗi ngày)
```yaml
nano /etc/systemd/system/add-vm-to-ha.timer
```

Dán:
```yaml
[Unit]
Description=Run add VM to HA daily at 02:00

[Timer]
OnCalendar=*-*-* 02:00:00
Persistent=true

[Install]
WantedBy=timers.target
```
## 9. Enable timer
```yaml
systemctl daemon-reload
systemctl enable --now add-vm-to-ha.timer
```
🔍 Kiểm tra timer
```yaml
systemctl list-timers | grep add-vm-to-ha
```
🧪 Test timer ngay (không đợi 2h sáng)
```yaml
systemctl start add-vm-to-ha.service
```

Xem log:
```yaml
journalctl -u add-vm-to-ha.service
```

## 10. Scripts trong file "/root/scripts/add_vm_to_ha.sh"

```yaml
#!/bin/bash
set -u

LOCK_FILE="/var/run/add_vm_to_ha.lock"
LOG_TAG="add_vm_to_ha"

echo "=== Bat dau quét máy ảo de thêm vào HA ==="

# ---------- KIỂM TRA LOCK ----------
if [ -f "$LOCK_FILE" ]; then
    echo "[$LOG_TAG] Script dang chay. Thoát."
    exit 1
fi
touch "$LOCK_FILE"
trap 'rm -f "$LOCK_FILE"' EXIT

# ---------- LẤY SNAPSHOT VM ----------
mapfile -t VM_ARRAY < <(
  pvesh get /cluster/resources --type vm --output-format json \
  | jq -c '.[]'
)

# ---------- VÒNG LẶP KIỂM TRA ----------
for vm in "${VM_ARRAY[@]}"; do
    vmid=$(echo "$vm" | jq -r '.vmid')
    node=$(echo "$vm" | jq -r '.node')
    template=$(echo "$vm" | jq -r '.template // 0')

    echo "[$LOG_TAG] Dang kiem tra VM $vmid tren node $node..."

    if [ "$template" = "1" ] || pvesh get /cluster/ha/resources | grep -q "\"vm:$vmid\""; then
        echo "  -> Bo qua (template hoac da co trong HA)"
        continue
    fi

    CONFIG=$(ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=5 root@"$node" "qm config $vmid" 2>/dev/null)

    if [ -z "$CONFIG" ] || echo "$CONFIG" | grep -Eq '^(scsi|virtio|sata|efidisk0|tpmstate0):\s*(local|local-lvm|local-zfs):'; then
        echo "  -> CHẶN: VM dung o dia cuc bo, khong an toan cho HA"
        continue
    fi

    echo "  -> Dang thêm VM $vmid vao HA..."
    pvesh create /cluster/ha/resources -sid "vm:$vmid" -state started && echo "  -> THANH CONG" || echo "  -> THAT BAI"
done
echo "=== Hoan thanh ==="
```
## Các bước triển khai
### Bước 1. Sửa định dạng và cấp quyền (Bắt buộc)
Chạy 2 lệnh sau để đảm bảo file sạch và có quyền thực thi:
```yaml
# Xóa ký tự xuống dòng lạ nếu bạn copy từ Windows
sed -i 's/\r$//' /root/scripts/add_vm_to_ha.sh

# Cấp quyền thực thi cho file
chmod +x /root/scripts/add_vm_to_ha.sh
```
### Bước 2. Kiểm tra trực tiếp (Test tay)
Đừng chạy service vội, hãy chạy trực tiếp script này để xem nó có báo lỗi gì không:
```yaml
/root/scripts/add_vm_to_ha.sh
```
### Bước 3. Khởi động lại Service
Bây giờ bạn có thể khởi động lại service qua systemd như trong hình bạn đã thử:
```yaml
systemctl daemon-reload
systemctl start add-vm-to-ha.service
journalctl -u add-vm-to-ha.service -f
```
